...
#' Use a sparse mapping matrix to optimize TPMs
#'
#' get optimal tpms using stan
#'
...
#' the columns values
#'
#' @param tpm_opt an object with an optimized stan TPM model
...
sample_cov_gr <- function(cov, tpms) {
  ...
  #
  iddf <- tibble(
    rind = 1:length(cov), j = names(cov) %>% id(),
    i = as.numeric(id(seqnames(cov)))
  )
  #
  rinds <- iddf %>%
...
gene_level_expr <- function(ripms, anno) {
  ...
  gn_expr <- left_join(
    trgiddf,
    tibble::enframe(ripms, "transcript_id", "tpm"),
    by = "transcript_id"
  )
  #
...
# TODOS remove 'TPM from the package'
...
get_exprfile <- function(ribobam, ribofasta, outfile) {
  ...
  tpmdf <- tibble::enframe(tpms, "Name", "TPM")
  #
